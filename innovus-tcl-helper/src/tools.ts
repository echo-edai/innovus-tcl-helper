 ...
 */
export function buildScriptContextForCommand(content: string, sourceLabel?: string): string {
    const db = getDB();
    return buildScriptContext(content, db, sourceLabel || (db.getLanguage() === 'zh' ? '当前脚本' : 'current script'));
}
