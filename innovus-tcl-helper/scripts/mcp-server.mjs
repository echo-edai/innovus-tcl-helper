...
function lintBuiltin(fFilePath,tclFiles){
  ...
  for(let o=0;o<scrs.length;o++){const{file,content}=scrs[o];const ls=content.split('\n');
    for(let li=0;li<ls.length;li++){const ln=ls[li].trim();if(!ln||ln.startsWith('#'))continue;const n=li+1;
...
function toolParse({script_content,script_path}){
  ...
  for(let i=0;i<ls.length;i++){const t=ls[i].trim();if(!t){bl++;continue;}if(t.startsWith('#')){cl++;continue;}const ft=t.split(/\s/)[0];if(!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(ft))continue;if(TB.has(ft))continue;
    const ci=getCmd(ft);
    if(ci&&ci.is_cmd!==false){const ps={};const tks=t.split(/\s+/);for(let ti=0;ti<tks.length;ti++){const tk=tks[ti];if(tk.startsWith('-')){const fg=tk.replace(/[,;]$/,'');ps[fg]=null;if(ti+1<tks.length&&!tks[ti+1].startsWith('-')){let v=tks[ti+1].replace(/[,;]$/,'');v=v.replace(/^\{/,'').replace(/\}$/,'');ps[fg]=v;ti++;}}}
